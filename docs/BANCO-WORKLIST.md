...
- **Catalog pass (2026-06-28, ON SANDBOX `feat/banco-pagination`, awaiting Angel test):** ✅ **infinite scroll** (IntersectionObserver, button fallback) · ✅ **Sort** (name/price/recent/stock, whitelisted server param) · ✅ **tap-to-PREVIEW** read-only detail modal (Edit-inside, manager-only). Seeded ~70 products into sandbox (`scripts/seed_sandbox_catalog.py`, transient) so it's testable. STAGING/PROD left as baseline for comparison. Then promote on approval.
