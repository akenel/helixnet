...
- [~] **P4 — Prod identity cleanup + SMTP.** 👥
  ...
  - [~] **Clean Banco POS realms — 3-realm rebuild IN PROGRESS.** Prod POS realm `borrowhood` holds **365 mostly-bot users** (old public-app pile); Felix's shop shouldn't share it. Plan = fresh clean realm per env, migrate real staff, quarantine the spam realm. (This is the `helix-identity-architecture` 3-realm plan; detail in `docs/IDENTITY-CONSOLIDATION-PLAN.md`.)
    - [x] **`kc-sandbox`** — already built + live (sandbox Banco runs on it).
    - [x] **`kc-staging` BUILT 2026-07-01 (additive)** — realm + 21 clients + tier/app roles + `shop:artemis` group, idempotent, **0 users**, nothing repointed (staging still on `borrowhood-staging`). Fully reversible. SMTP skeleton mirrors staging; ⏳ Resend key to be pasted at fold time (needs Angel — `configure_kc_smtp.py`).
    - [ ] **Fold staging staff → `kc-staging` + cutover** (gated, with Angel): migrate real staff (handover pattern, complete profiles), set Resend key, one-env-var cutover (`POS_REALM`/`LP_REALM`), login smoke, one-var rollback ready. Then retire `borrowhood-staging`.
    - [ ] **`kc-production`** — build additively, fold real staff, gated cutover off `borrowhood` (own quiet window, backup first). Quarantine the 365-user swamp.
