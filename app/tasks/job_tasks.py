...
from app.services.job_processing_service import job_processor 
...
def process_job(self, job_id: str) -> bool:
    ...
    try:
        ...
        with get_db_worker() as db:
            update_job_status_for_celery(
                db, 
                job_uuid, 
                {'status': Job.Status.IN_PROGRESS, 'started_at': datetime.utcnow()}
            )
    ...
    final_status = Job.Status.COMPLETED if success else Job.Status.FAILED
    update_data = {
        'status': final_status, 
