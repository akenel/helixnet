#!/usr/bin/env python3
"""Born Once #16 — "The Snag" (Season 3). App frames (the BEFORE Transaction History,
no export) with red caption bars + Bauhaus-on-noir title cards, crossfaded on the
VO16_final beat map (Whisper), Brotherhood Run at 5%. Mirrors build_cream03_v2 + build_14."""
import os, subprocess
from PIL import Image, ImageDraw, ImageFont

ROOT = "/home/angel/repos/helixnet"
PROJ = f"{ROOT}/videos/banco/Season 3 - Never Alone/born-once-16-the-button"
SHOTS = f"{PROJ}/assets/shots"
SLIDES = f"{PROJ}/assets/slides"
VO = f"{PROJ}/VO16_final.wav"
MUSIC = f"{ROOT}/compose/helix-media/music/angel-originals/Brotherhood Run.mp3"
OUT = f"{PROJ}/Banco-BornOnce-16-The-Button-DRAFT.mp4"
...
# id, type, frame, text, window(s) — aligned to VO16_final beat map (Whisper segments)
TL = [
    ("s01", "card", None,             "He couldn't keep\nthe day.",                  8.00),
    ("s02", "card", None,             "The one thing\nhe needed.",                   7.96),
    ("s03", "card", None,             "The old way:\nopen a ticket.",               12.04),
    ("s04", "card", None,             "Wait behind ten\nthousand strangers.",        9.06),
    ("s05", "app",  "fb-01-button",   "He taps the bubble\nin the corner.",          7.96),
    ("s06", "app",  "fb-02-card",     "Bug, or idea?",                               7.64),
    ("s07", "app",  "fb-02-card",     "How bad? Blocking,\nannoying, cosmetic.",     8.04),
    ("s08", "app",  "fb-03-typed",    "He says it in\nhis own words.",               9.30),
    ("s09", "app",  "fb-03-typed",    "No jargon.\nHe taps 'annoying'.",             8.00),
    ("s10", "app",  "fb-04-filed",    "He hits send.",                               7.00),
    ("s11", "app",  "fb-04-filed",    "Filed — straight\nto the workbench.",         9.10),
    ("s12", "card", None,             "Now the brain\nwakes up.",                   11.50),
    ("s13", "outro", None,            "#16 · The Button",                            3.40),
]
...
def brand(d):
    ...
    d.text((262, H - 50), "·  #16  ·  Banco", font=font(FR, 26), fill=MUTED, anchor="lm")
...
print(f"{len(paths)} slides, total {total:.1f}s (VO≈105.6s)")
