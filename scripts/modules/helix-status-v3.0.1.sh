...
declare -A PORTS=(
  ...
  [prometheus]=9090 [dozzle]=8080 [grafana]=3000 [mailhog]=8025 [keycloak]=8080 [minio]="9000/1" [portainer]=9443 [vault]=8200 [traefik]="80/443" [filebrowser]=80
)
# --- 5. RENDER FUNCTIONS ---
# Header renderer
render_header() {
...
render_cycle() {
  ...
  docker ps -a --format "{{.Names}}\t{{.Status}}\t{{.ID}}" | while IFS=$'\t' read -r name status container_id; do
    ...
    case "$service_name" in
        ...
        # n8n)          ICONS="📢";  desc_base="n8n 📢 Automation, Webhook Workflow" ;;         # portainer)    ICONS="📺️";  desc_base="portainer ◾ Container UI          " ;;
        # n8n)          ICONS="📢";  desc_base="n8n 📢 Automation, Webhook Workflow" ;; 
