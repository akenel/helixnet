...
(async () => {
  ...
  await sleep(2500);
  await page.evaluate(() => window.scrollTo({ top: 600, behavior: 'smooth' })); await sleep(2500);   // pan the recipe cards
  ...
  for (let k = 0; k < frames.length; k++) {
    const dur = k < frames.length - 1
      ? Math.min(2.0, Math.max(0.033, frames[k + 1].t - frames[k].t)) : 0.5;
  ...
  execSync(`ffmpeg -y -loglevel error -f concat -safe 0 -i ${FRAMES}/list.txt -vf "fps=24,format=yuv420p,scale=1920:1080" -c:v libx264 -preset medium -crf 20 "${OUT}"`, { stdio: 'inherit' });
