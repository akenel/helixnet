// Ep 11 — "Saturday: The Lift" (THE CLIMAX) -> MP4. Silent, natural, zoomed.
// The crew came (the roster + their cookie notes) and the ask is RESOLVED - the crane rose.
...
const FRAMES = '/tmp/ep11rec';
const OUTDIR = '/home/angel/Videos/dream-weavers';
const OUT = `${OUTDIR}/ep11-playthrough.mp4`;
...
(async () => {
  ...
  // scene 1: the event - the crew came (roster + cookie notes)
  ...
  await sleep(3200);                                            // the event - the count climbed (6/12, the crew came)
  await page.evaluate(() => window.scrollTo({ top: 640, behavior: 'smooth' })); await sleep(2400);   // the attendees panel
  await page.evaluate(() => { const b = [...document.querySelectorAll('button')].find(x => /attendee/i.test(x.textContent) && (x.getAttribute('@click') || '').includes('open')); if (b) b.click(); });
  await sleep(4800);                                            // the roster expands: the crew + their cookie notes
  await page.evaluate(() => window.scrollTo({ top: 980, behavior: 'smooth' })); await sleep(3400);   // more of who showed up
  await page.evaluate(() => window.scrollTo({ top: 0, behavior: 'smooth' })); await sleep(1800);     // back: the ask, answered
