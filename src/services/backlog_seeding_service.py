...
SEED_ITEMS = [
    ...
    (
        BacklogItemType.BUG_FIX, BacklogStatus.DONE, BacklogPriority.HIGH,
        "BUG-010 upload images (MinIO + auth)",
        "Fix document preview auth -- use JS fetch with Bearer token for all doc ops.",
        "Tigs", "bug,minio,auth",
    ),
    (
        BacklogItemType.DEV_TASK, BacklogStatus.DONE, BacklogPriority.MEDIUM,
    ...
    (
        BacklogItemType.BUSINESS_OPS, BacklogStatus.PENDING, BacklogPriority.MEDIUM,
    ...
    (
        BacklogItemType.BUSINESS_OPS, BacklogStatus.PENDING, BacklogPriority.LOW,
