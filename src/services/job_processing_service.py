...
from src.schemas.job_schema import JobUpdate # Used for type checking update_job_status_for_celery
...
# from src.services.job_processing_service import get_job_by_id_sync 
from src.services.minio_service import minio_service # Sync MinIO client
...
def get_job_by_id_sync(db: Session, job_id: uuid.UUID) -> Optional[JobModel]:
    """
    ...
    job_record = db.get(JobModel, job_id)
...
def update_job_status_for_celery(db: Session, job_id: uuid.UUID, update_data: Dict[str, Any]) -> None:
    ...
    if result.rowcount == 0:
        logger.warning(f"DB SYNC: Update failed. JobMODEL {job_id} not found or no changes made.")
        ...
        db.commit() 
        logger.info(f"DB SYNC: JobMODEL {job_id} updated successfully.")
