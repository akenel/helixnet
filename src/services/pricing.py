...
def tier_unit_price(price_tiers, base_price, qty):
    """Resolve the unit price for ``qty`` from ``price_tiers``.

    Returns ``(unit_price: Decimal, volume_break: bool)``. The winning tier is the one
    with the highest ``min_qty`` that is still ``<= qty``. ``volume_break`` is True only
    when that tier is a genuine quantity break (``min_qty >= 2``) — which flags the line
    as discount-final. Falls back to ``base_price`` when there are no tiers or none apply.
    """
    base = _q(base_price)
    if not price_tiers:
        return base, False
    ...
    for t in price_tiers:
        ...
        if best_qty is None or mq > best_qty:
            best_qty, best_price = mq, up
    if best_qty is None:
        return base, False
    return _q(best_price), best_qty >= 2
