...
import requests
from src.db.session import SyncSessionLocal   # a synchronous session factory
...
def process_job(job_id: str):
    ...
    try:
        job: JobModel = db.get(JobModel, job_id)
        if not job:
        ...
        update_job_status_for_celery(
            db,
            job.job_id,
            {"status": "COMPLETED", "finished_at": datetime.now(tz=timezone.utc)},
        )
